...
function pre_install() {
    ...
    HOSTNMAE=`hostname`
    var=`cat /root/queens_controller_compute/conf_controller |grep ip`
    IP=${var#*=}
...
function mariadb_install () {
    ...
    varp=`cat /root/queens_controller_compute/conf_controller |grep password`
    PASS=${varp#*=}
    ...
    if [ ! -f /root/queens_controller_compute/sql.tag ]
    then
    ...
    y
EOF
    touch /root/queens_controller_compute/sql.tag
    fi
    res "设置数据库密码$PASS"
}
...
function neutron_install () {
    ...
    varn=`cat /root/queens_controller_compute/conf_controller |grep net`
...
function dashboard_install () {
   ...
   # rm -rf /etc/openstack-dashboard/local_settings >>/dev/null
    ...
    cp /root/queens_controller_compute/local_settings.py /etc/openstack-dashboard/
...
function main(){
PS3="选择序号 : "
select i in "pre_install" "ntp_install" "client_install" "mariadb_install" "rabbitmq_install" "memcached_install" "etcd_install" "keystone_install" "glance_install" "nova_install" "neutron_install" "dashboard_install" "quit"
do
case $i in 
	pre_install   )
	pre_install
