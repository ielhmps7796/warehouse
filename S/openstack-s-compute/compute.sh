...
#    if [ ${HOSTNAME}x != computex ]
    ...
    cat /etc/hosts |grep compute1 >>/dev/null
    if [ $? -eq 1 ]
    then
...
function nova_install () {
    ...
    varp=`cat /root/openstack-s/compute_conf |grep password`
...
#    sed -i "s/os_region_name\ =\ openstack/os_region_name\ =\ RegionOne/g" /etc/nova/nova.conf
    add_line /etc/nova/nova.conf "\[placement\]$" "region_name = RegionOne"
...
function neutron_install () {
    ...
    varn=`cat /root/openstack-s/compute_conf |grep net`
...
function swift_install () {
    ...
    apt install -y xfsprogs rsync >>/dev/null

    res "安装swift软件包"

   DISK=`fdisk -l | grep "^Disk /dev/sd[b-z]" | cut -d / -f 3 | cut -d : -f 1 | sort`
   for DEVICE in $DISK
   do
	mkfs.xfs -f /dev/$DEVICE
	if [ ! -d /srv/node/$DEVICE ]
	then
		mkdir -p /srv/node/$DEVICE
	fi
	echo "/dev/$DEVICE /srv/node/$DEVICE xfs noatime,nodiratime,nobarrier,logbufs=8 0 2" >> /etc/fstab
	mount /srv/node/$DEVICE
	echo -e "\033[32m $DEVICE 完成\033[0m"
   done
...
EOF
   ...
   apt-get install swift swift-account swift-container swift-object
    ...
    if [ ! -f /etc/swift/account-server.conf.bak ]
    then
 	cp /etc/swift/account-server.conf /etc/swift/account-server.conf.bak
    fi
    add_line /etc/swift/account-server.conf "\[DEFAULT\]$" "bind_ip = $IP"
    add_line /etc/swift/account-server.conf "\[DEFAULT\]$" "user = swift"
    add_line /etc/swift/account-server.conf "\[DEFAULT\]$" "swift_dir = /etc/swift"
    add_line /etc/swift/account-server.conf "\[DEFAULT\]$" "devices = /srv/node"
    add_line /etc/swift/account-server.conf "\[DEFAULT\]$" "mount_check = True"
    add_line /etc/swift/account-server.conf "\[filter:recon\]$" "recon_cache_path = /var/cache/swift"

    if [ ! -f /etc/swift/container-server.conf.bak ]
    then
	cp /etc/swift/container-server.conf /etc/swift/container-server.conf.bak
    fi
    add_line /etc/swift/container-server.conf "\[DEFAULT\]$" "bind_ip = $IP"
    add_line /etc/swift/container-server.conf "\[DEFAULT\]$" "user = swift"
    add_line /etc/swift/container-server.conf "\[DEFAULT\]$" "swift_dir = /etc/swift"
    add_line /etc/swift/container-server.conf "\[DEFAULT\]$" "devices = /srv/node"
    add_line /etc/swift/container-server.conf "\[DEFAULT\]$" "mount_check = True"
    add_line /etc/swift/container-server.conf "\[filter:recon\]$" "recon_cache_path = /var/cache/swift"

    if [ ! -f /etc/swift/object-server.conf.bak ]
    then
	cp /etc/swift/object-server.conf /etc/swift/object-server.conf.bak
    fi
    add_line /etc/swift/object-server.conf "\[DEFAULT\]$" "bind_ip = $IP"
    add_line /etc/swift/object-server.conf "\[DEFAULT\]$" "user = swift"
    add_line /etc/swift/object-server.conf "\[DEFAULT\]$" "swift_dir = /etc/swift"
    add_line /etc/swift/object-server.conf "\[DEFAULT\]$" "devices = /srv/node"
    add_line /etc/swift/object-server.conf "\[DEFAULT\]$" "mount_check = True"
    add_line /etc/swift/object-server.conf "\[filter:recon\]$" "recon_cache_path = /var/cache/swift"
    add_line /etc/swift/object-server.conf "\[filter:recon\]$" "recon_lock_path = /var/lock"

    chown -R swift:swift /srv/node
    if [ ! -d /var/cache/swift ]
    then
        mkdir -p /var/cache/swift
    	chown -R root:swift /var/cache/swift
    	chmod -R 775 /var/cache/swift
    else
 	echo -e "\033[32m 目录已存在\033[0m"
    fi

    echo -e "\033[32m接下来在controller节点上运行swift_CreateRing.sh\033[0m"
}


function main(){
PS3="请选择序号 ： "
select i in "pre_install" "ntp_install" "client_install" "nova_install" "neutron_install" "swift_install" "quit"
do 
case $i in
